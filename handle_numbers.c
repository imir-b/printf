...
int	ft_print_int(long long n, int zeros, char sign, t_struct *list)
{
	int	len;
	int	ret_len;
	...
	t_struct *list)
{
	...
	if (list->is_hash && n != 0)
		prefix_len = 2;
	len = ft_nbaselen(n, ft_strlen(base)) + zeros + prefix_len;
	if (n == 0 && list->precision == 0)
		len = 0;
	if (!list->is_minus && !list->is_zero)
		ret_len += ft_put_padding(list->width, len, ' ', list);
	if (prefix_len && ft_isupper(base))
...
int	ft_handle_nbrbase(unsigned int nb, const char *base, t_struct *list)
{
	...
	if (list->precision >= 0)
		list->is_zero = 0;
	len = ft_nbaselen(n, ft_strlen(base));
	if (n == 0 && list->precision == 0)
		len = 0;
	zeros = 0;
	if (list->precision > len)
